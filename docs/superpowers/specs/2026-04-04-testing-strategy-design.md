...
> **참고:** shadcn/ui 전환이 결정됨에 따라, UI 컴포넌트 테스트는 전환 이후에 작성한다.
> core 테스트를 먼저 완성하여 전환 시 비즈니스 로직을 보호한다.

1. 테스트 인프라 구성 (Vitest Workspace, 로컬 Supabase, 트랜잭션 헬퍼)
2. utils 기능 정의서 → 테스트 → 코드 정리
3. stores 기능 정의서 → 테스트 → 코드 정리
4. **shadcn/ui 전환** (별도 스펙: `2026-04-04-shadcn-migration-design.md`)
5. UI 컴포넌트 기능 정의서 → 테스트 → 코드 정리 (shadcn/ui 기준)
6. E2E 시나리오 확장
7. 릴리즈 스크립트 통합
8. 전체 커버리지 확인 및 최종 검증
