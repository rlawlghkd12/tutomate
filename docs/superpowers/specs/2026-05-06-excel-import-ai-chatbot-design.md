...
### 사용자 요청
1. 학원이 보유한 엑셀 파일을 업로드하면 수강생/납입 정보를 자동으로 추가
2. 자연어 질의로 학생 데이터 조회·요약 (예: "민준이 결제 언제 했더라?")

### 핵심 제약
- 60대 이상 사용자가 다수 — 직관성/큰 글씨/단순 UX 필수
- 학원 데이터(학생 이름·전화·결제)에 개인정보 포함 → **외부 전송 불가**
- 운영자가 사용량 기반 클라우드 비용·API 키 관리 부담을 지지 않음
- 학원 PC는 8GB RAM Windows가 일반 (16GB 미보장)

### 목표
- 엑셀 임포트와 AI 챗봇을 **한 번에** 출시
- 풀로컬 LLM, 온디맨드 다운로드, 외부 전송 0
- 모든 학원 PC에서 엑셀 임포트는 동작, 챗봇은 사양 게이트
...
- `name`, `phone`, `parentPhone`, `birthDate`, `enrollmentDate`,
  ...
  `className` (수강반), `tuitionPlan` (수강 과정)

매핑 알고리즘:
1. 헤더 정규화: 공백/괄호/특수문자 제거, 소문자화
2. **학원별 캐시 조회** (`mapping_profiles` 테이블, key = sha1(정규화 헤더 시퀀스))
   - HIT → 즉시 적용, 끝
3. **동의어 사전 매칭**:
   - 정확 일치 → 매핑 확정
   - 부분 일치(포함) → 매핑 확정
...
| `getStudent` | `studentId: string` | 학생 상세 |
...
| `listClasses` | `studentId?: string` | 수강반 목록 |
| `getClassRoster` | `classId: string` | 반 학생 명단 |
| `getMonthlySummary` | `month: 'YYYY-MM'` | 해당 월 매출/등록 요약 |
| `getStudentSummary` | `studentId: string` | 학생 종합 요약 (자연어) |
...
| ColumnMapper | 동의어 사전 매칭, 미매칭 검출, 캐시 HIT/MISS | 빠름 |
...
5. 엑셀 임포트 UI: 업로드 → 매칭 결과 → 미리보기 → 확정 (60대 친화)
...
- **다국어**: 현재 한국어 전용. 일본어/베트남어 학원 요구 시 별도 검토.
- **챗봇 히스토리**: 대화 영속화 여부 (학원별 / 사용자별 / 세션 한정)
