...
describe('courseStore', () => {
  ...
  it('addCourse → state에 추가, currentStudents 0 초기화', async () => {
    await useCourseStore.getState().addCourse({
      name: '영어', classroom: 'B201', instructorName: '박강사',
      instructorPhone: '010-0000-0000', fee: 250000, maxStudents: 20,
    });
  ...
  it('updateCourse → 부분 업데이트, 다른 필드 유지', async () => {
    ...
    await useCourseStore.getState().updateCourse('c1', { name: '고급수학', fee: 400000 });
  ...
  it('incrementCurrentStudents → 1 증가', async () => {
    useCourseStore.setState({ courses: [makeCourse({ currentStudents: 5, maxStudents: 30 })] });
    await useCourseStore.getState().incrementCurrentStudents('c1');
    expect(useCourseStore.getState().getCourseById('c1')!.currentStudents).toBe(6);
  });

  it('incrementCurrentStudents → maxStudents 도달 시 증가 안됨', async () => {
    useCourseStore.setState({ courses: [makeCourse({ currentStudents: 30, maxStudents: 30 })] });
    await useCourseStore.getState().incrementCurrentStudents('c1');
    expect(useCourseStore.getState().getCourseById('c1')!.currentStudents).toBe(30);
  });
  ...
  it('decrementCurrentStudents → 1 감소', async () => {
    ...
    expect(useCourseStore.getState().getCourseById('c1')!.currentStudents).toBe(4);
  });

  it('decrementCurrentStudents → 0이면 감소 안됨', async () => {
    useCourseStore.setState({ courses: [makeCourse({ currentStudents: 0 })] });
    await useCourseStore.getState().decrementCurrentStudents('c1');
    expect(useCourseStore.getState().getCourseById('c1')!.currentStudents).toBe(0);
  });
