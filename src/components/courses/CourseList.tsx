...
const CourseList: React.FC = () => {
  ...
  const handleDelete = (course: Course) => {
    if (course.currentStudents > 0) {
      Modal.confirm({
        ...
        content: (
          <div>
            <p><strong>{course.name}</strong> 강좌에 현재 <strong style={{ color: '#ff4d4f' }}>{course.currentStudents}명</strong>의 수강생이 등록되어 있습니다.</p>
  ...
  const getStatus = (course: Course) => {
    if (course.currentStudents >= course.maxStudents) {
      return 'full';
    } else if (course.currentStudents >= course.maxStudents * 0.8) {
      return 'almost';
    } else {
      return 'open';
    }
