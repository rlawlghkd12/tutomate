/**
 * 로컬 파일시스템(Electron) 데이터 → Supabase 클라우드 마이그레이션 헬퍼
 *
 * 프로덕션 환경에서는 Electron 파일시스템({key}.json)에만 데이터가 존재.
 * 비-Electron(브라우저) 환경은 항상 Supabase를 직접 사용하므로 마이그레이션 불필요.
 */
...
/**
 * Electron 파일시스템에서 JSON 배열 로드
 * Electron 환경이 아니면 빈 배열 반환
 */
async function loadFromLocal<T>(key: string): Promise<T[]> {
  if (!isElectron()) return [];
  try {
    const raw = await window.electronAPI.loadData(key);
    if (raw) {
...
export async function hasLocalData(): Promise<boolean> {
  ...
  try {
    const courses = await loadFromLocal<Course>('courses');
    if (courses.length > 0) return true;
    const students = await loadFromLocal<Student>('students');
...
/**
 * Electron 파일시스템의 로컬 데이터를 Supabase로 마이그레이션
 *
...
export async function migrateLocalToCloud(
  ...
  const courses = await loadFromLocal<Course>('courses');
  const students = await loadFromLocal<Student>('students');
  const enrollments = await loadFromLocal<Enrollment>('enrollments');
  const monthlyPayments = await loadFromLocal<MonthlyPayment>('monthly_payments');
...
export async function clearLocalData(): Promise<void> {
  ...
  try {
    await Promise.all(
      keys.map((key) =>
        window.electronAPI.saveData(key, '[]').catch((err) =>
          logWarn(`Failed to clear local file for "${key}": ${err}`),
        ),
      ),
    );
  } catch (err) {
    logWarn(`Electron API failed during clearLocalData: ${err}`);
  }
