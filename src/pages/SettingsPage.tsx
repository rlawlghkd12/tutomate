...
import { APP_NAME, useAppVersion } from "../config/version";
...
const SettingsPage: React.FC = () => {
	...
	const { getPlan, activateLicense, deactivateLicense, licenseKey } =
		useLicenseStore();
	...
	const handleCheckUpdate = async () => {
		if (!isElectron()) {
			message.warning("업데이트는 데스크톱 앱에서만 사용 가능합니다");
			return;
		}
		...
		try {
			...
			if (result && result.version !== APP_VERSION) {
				...
				message.info(
					`새 버전 v${result.version}이 있습니다! (현재: v${APP_VERSION})`,
				);
			} else {
				setIsLatest(true);
			...
			message.error(`업데이트 확인 실패: ${msg} (현재: v${APP_VERSION})`);
		} finally {
			...
			if (result.result === "success") {
				if (result.orgChanged) {
					Modal.confirm({
						...
						content:
							"이 라이선스 키에는 기존 데이터가 있습니다. 전환하면 현재 체험판에서 입력한 데이터는 더 이상 표시되지 않습니다. 계속하시겠습니까?",
						okText: "전환",
						cancelText: "취소",
						onCancel: () => {
					...
					<div style={settingRowStyle}>
						...
						<Space>
							...
							<Button
								...
								size="small"
								disabled={
									currentPlan === "trial" || orgNameInput === organizationName
								}
								onClick={async () => {
								...
								<Button
									...
									onClick={() => {
										Modal.confirm({
											...
											content:
												"로그아웃하면 클라우드 동기화가 해제됩니다. 계속하시겠습니까?",
											okText: "로그아웃",
											okType: "danger",
											cancelText: "취소",
											onOk: async () => {
