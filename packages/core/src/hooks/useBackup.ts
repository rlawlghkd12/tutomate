...
// toast 콜백 — UI 레이어에서 주입
let _toast: { success: (msg: string) => void; error: (msg: string) => void; warning: (msg: string) => void } = {
  success: (msg) => console.log('[backup]', msg),
  error: (msg) => console.error('[backup]', msg),
  warning: (msg) => console.warn('[backup]', msg),
};

export function setBackupToast(t: typeof _toast) { _toast = t; }
...
export function useBackup() {
  ...
  const loadBackups = useCallback(async () => {
    ...
    try {
      ...
      _toast.error('백업 목록을 불러오는데 실패했습니다: ' + error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createBackup = useCallback(async () => {
    if (!isElectron()) { _toast.warning('백업은 데스크톱 앱에서만 사용 가능합니다'); return; }
    setCreating(true);
    try {
      await createCloudBackup(organizationName);
      _toast.success('백업이 성공적으로 생성되었습니다');
      await loadBackups();
    } catch (error) {
      _toast.error('백업 생성에 실패했습니다: ' + error);
    } finally {
      setCreating(false);
    }
  ...
  const importBackup = useCallback(async (): Promise<BackupInfo | null> => {
    if (!isElectron()) { _toast.warning('백업은 데스크톱 앱에서만 사용 가능합니다'); return null; }
    try {
      ...
      _toast.error('백업 파일 불러오기 실패: ' + error);
      return null;
    } finally {
      setImporting(false);
    }
  ...
  const restoreBackup = useCallback(async (filename: string) => {
    ...
    if (!orgId) {
      _toast.error('조직 정보가 없어 복원할 수 없습니다.');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        _toast.success('백업이 성공적으로 복원되었습니다. 페이지를 새로고침합니다.');
        setTimeout(() => window.location.reload(), 1500);
      } else {
        _toast.error('백업 복원에 실패했습니다: ' + (result.error || '알 수 없는 오류'));
        setRestoring(null);
      }
    } catch (error) {
      _toast.error('백업 복원에 실패했습니다: ' + error);
      setRestoring(null);
    }
  }, []);
  ...
  const downloadBackup = useCallback(async (filename: string) => {
    ...
    try {
      ...
      _toast.success('백업 파일이 저장되었습니다.');
    } catch (error) {
      _toast.error('백업 파일 다운로드 실패: ' + error);
    }
  }, []);
  ...
  const deleteBackup = useCallback(async (filename: string) => {
    ...
    try {
      ...
      _toast.success('백업이 삭제되었습니다');
      await loadBackups();
    } catch (error) {
      _toast.error('백업 삭제에 실패했습니다: ' + error);
    }
  }, [loadBackups]);
