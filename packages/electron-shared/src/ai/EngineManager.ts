 ...
 * 설치 위치: userData/AI/llama-bin/<platform>/  (findLlamaServerBin #2 경로와 일치)
 */
export class EngineManager {
  ...
  isInstalled(): boolean {
    return findLlamaServerBin(this.userDataDir, this.resourcesPath) !== null;
  }

  private destDir(): string | null {
    const platform = detectPlatformDir();
    if (!platform) return null;
    return path.join(this.userDataDir, 'AI', 'llama-bin', platform);
  }
